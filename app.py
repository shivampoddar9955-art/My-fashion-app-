...
with st.sidebar.form("product_form", clear_on_submit=True):
    ...
    submit_p = st.form_submit_with_button("Add to Collection")

    if submit_p and p_name and cp > 0:
        if p_name in products_df["Item/Design Name"].values:
            st.sidebar.error("Item already exists!")
        else:
            sp = cp * 1.20
            ...
            st.sidebar.success(f"Added! SP: ₹{sp:.2f}")
            st.rerun()

# --- SIDEBAR: LOG SALE ---
st.sidebar.write("---")
st.sidebar.header("🛒 Record a Sale")
if not products_df.empty:
    with st.sidebar.form("sale_form", clear_on_submit=True):
        selected_p = st.selectbox("Select Sold Item", products_df["Item/Design Name"].tolist())
        qty = st.number_input("Quantity Sold", min_value=1, step=1)
        submit_s = st.form_submit_with_button("Log Sale")
        ...
        if submit_s:
            ...
            save_data(products_df, sales_df)
            st.sidebar.success("Sale Recorded!")
            st.rerun()
...
if not sales_df.empty:
    ...
    col3.metric("Net Profit", f"₹{total_prof:,.2f}", delta="20% Margin")
    col4.metric("Clothes Sold", int(sales_df["Quantity Sold"].sum()))
else:
    col1.metric("Total Sales", "₹0.00")
    col2.metric("Total Cost", "₹0.00")
    col3.metric("Net Profit", "₹0.00")
    col4.metric("Clothes Sold", 0)

st.write("---")
tab1, tab2 = st.tabs(["📊 Sales History", "📦 Stock/Inventory"])
with tab1:
